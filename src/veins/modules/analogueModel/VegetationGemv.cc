...
VegetationGemv::VegetationGemv(VegetationObstacleControl& vegetationObsCtl, double carrierFrequency, const Coord& playgroundSize, bool debug) :
...
double VegetationGemv::calcAttenuation(const Coord& senderPos, const Coord& receiverPos) const
{
	...
	double mel=0.79*pow(fInGhz,0.61);
	L = 0.45*pow(fInGhz,0.284)*depthInVegetation;
	/*
	if(depthInVegetation >= 0 && depthInVegetation <= 14)
	{
		L = 0.45*pow(fInGhz,0.284)*depthInVegetation;
//		std::cout<<"frequency "<<fInGhz<<", depth in Vegetation "<<depthInVegetation<<std::endl;
...
void VegetationGemv::filterSignal(AirFrame* frame, const Coord& sendersPos, const Coord& receiverPos)
{
