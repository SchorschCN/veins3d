...
VegetationCOST::VegetationCOST(VegetationObstacleControl& vegetationObsCtl, double carrierFrequency, const Coord& playgroundSize, bool leaf, bool debug) :
...
double VegetationCOST::calcAttenuation(const Coord& senderPos, const Coord& receiverPos) const
{
    ...
    if(fInMhz>=9600 && fInMhz<=57600)
    {
        if(leaf==false)
        {
            L = 26.6*pow(fInMhz,-0.2)*pow(depthInVegetation,0.5);
        }
        else
        {
            L = 15.6*pow(fInMhz,-0.009)*pow(depthInVegetation,0.26);
        }
    }
    else
    {
...
void VegetationCOST::filterSignal(AirFrame* frame, const Coord& sendersPos, const Coord& receiverPos)
{
